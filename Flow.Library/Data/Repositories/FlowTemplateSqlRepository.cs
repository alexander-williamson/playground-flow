...
namespace Flow.Library.Data.Repositories
{
    public class FlowTemplateSqlRepository : IFlowTemplateRepository
    {
        ...
        public IEnumerable<FlowTemplate> Get(IDbTransaction transaction = null)
        {
            if(_connection.State == ConnectionState.Closed) 
                _connection.Open();

            return _connection.Query<FlowTemplate>("SELECT * FROM FlowTemplate", null, transaction);
        }
        ...
        public FlowTemplate Get(int id, IDbTransaction transaction = null)
        {
            ...
            var result = _connection.Query<FlowTemplate>("SELECT * FROM FlowTemplate WHERE Id = @Id", new { Id = id }, transaction).ToArray();
            return result.First();
        }

        public int Add(FlowTemplate instance, IDbTransaction transaction = null)
        {
            if(_connection.State == ConnectionState.Closed) 
                _connection.Open();
        ...
        public void Update(int id, FlowTemplate instance, IDbTransaction transaction = null)
        {
            if (_connection.State == ConnectionState.Closed)
                _connection.Open();
